 ...
 * @Last Modified time: 2014-12-16 01:43:00
 */
			...
			if (__.size(itemsData) > 0) {
				...
				async.map(temparray, function (data, callback) {
					...
					if(items.type == 'insertion') {
						...
						grabber.release(results, items.translate.insertion, function(err, data) {

							if(err) {

								log.message('Error on releasing ' + items.name)
						...
						grabber.release(results, items.translate.deletion, function(err, data){
