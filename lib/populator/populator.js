	...
	module.exports = function () {
		...
		populator.init = function(){
			...
			ev.subscribe('populatorPopulate', function(data){
				...
				async.map(data.data, function(_data) {

					data.callback(_data, function(err, status) {

						console.log('ini return dari callback config', status)

					})

				})

				// populator.storing('tmp', data.data).populate('tmp', function(err, data){
