				...
				setTimeout(function () {
					...
					async.map(conf, analyzer.compare, function (err, results) {
						...
						async.each(results, function (data, callback) {
							...
							log.message('Analyzer :: Table ' + __.keys(nm)[0] + ' with ' + nm[__.keys(nm)[0]] + ' vs ' + nm[__.keys(nm)[1]])
							...
							if (num > 0 || num < 0) {
								...
								return ev.publish('synck.analyzer.differ', {
		...
		analyzer.compare = function (item, callback) {
			...
			async.parallel(tasks, function (err, results) {
				...
				if (err) {
					...
					log.message('Analyzer :: Comparation error', 'error')

					console.log(err)
				}

				// log.message('Analyzer :: Comparation done with no error')
